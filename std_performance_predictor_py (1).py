...
df = pd.read_csv("/Student_performance_data _.csv")
    # Apply your cleaning steps here if they aren't already done
    # Identify and handle outliers (using winsorizing as in your notebook)


# --- Define Features and Target ---
# Ensure these lists match your data and definitions
categorical_binary = ['Gender', 'Tutoring', 'Extracurricular', 'Sports', 'Music', 'Volunteering']
categorical_multi = ['Ethnicity', 'ParentalEducation', 'ParentalSupport', 'GradeClass'] # Assuming ParentalSupport is multi-category
...
# Corrected the handle_unknown parameter to a valid value
...
# Combine all transformers into one column transformer
# We use 'remainder='passthrough'' to ensure numerical columns are kept before scaling
preprocessor = ColumnTransformer(
    ...
    remainder='passthrough' # Keep any columns not listed above (e.g., numerical before scaling)
)

# --- Create the Pipeline with Preprocessor and RandomForestRegressor ---
# The pipeline includes the preprocessing steps and the adopted model
pipeline_forest = Pipeline(steps=[
    ("preprocessor", preprocessor), # Step 1: Apply preprocessing
    ("regressor", RandomForestRegressor(random_state=42)) # Step 2: The chosen model
])
...
pipeline_forest.fit(X_train, y_train) # Fits the preprocessor and the regressor
...
print(f"Random Forest Pipeline - Test MSE: {test_mse:.4f}")
print(f"Random Forest Pipeline - Test R2: {test_r2:.4f}")

# --- Save the Trained Pipeline ---
pipeline_filename = "best_student_performance_pipeline.pkl"
joblib.dump(pipeline_forest, pipeline_filename)

print(f"\n✅ Trained pipeline saved as {pipeline_filename}")

# --- Make the .pkl file available for download ---
print(f"Making '{pipeline_filename}' available for download...")
try:
    files.download(pipeline_filename)
    print(f"'{pipeline_filename}' is ready for download.")
